...
(function() {
    ...
    if ('fonts' in document) {
        // Загружаем критически важные шрифты
        const criticalFonts = [
            new FontFace('Inter', 'url(fonts/Inter-Regular.woff2)', { weight: '400', display: 'swap' }),
            new FontFace('Inter', 'url(fonts/Inter-SemiBold.woff2)', { weight: '600', display: 'swap' })
        ];
        ...
        Promise.all(
            ...
            // Загружаем остальные шрифты после критических
    ...
    function loadNonCriticalFonts() {
        const nonCriticalFonts = [
            new FontFace('Inter', 'url(fonts/Inter-Light.woff2)', { weight: '300', display: 'swap' }),
            new FontFace('Inter', 'url(fonts/Inter-Medium.woff2)', { weight: '500', display: 'swap' }),
            new FontFace('MontserratAlternates', 'url(fonts/MontserratAlternates-SemiBold.woff2)', { weight: '600', display: 'swap' }),
            new FontFace('MontserratAlternates', 'url(fonts/MontserratAlternates-Bold.woff2)', { weight: '700', display: 'swap' })
        ];

        nonCriticalFonts.forEach(font => {
            document.fonts.add(font);
            font.load().catch(() => {
                console.warn('Failed to load non-critical font:', font.family);
            });
        });
    }
})();
...
// 6. Последовательная загрузка JavaScript библиотек
async function loadScripts() {
    try {
        // Проверяем, не загружен ли уже jQuery
        if (typeof jQuery === 'undefined') {
            await loadScriptAsync('script/jquery-3.7.0.min.js');
        }
        
        // Загружаем Swiper только если нужен
        if (document.querySelector('.swiper')) {
            await loadScriptAsync('script/swiper-bundle.min.js');
        }
        
        // Загружаем основной скрипт последним
        await loadScriptAsync('script/main.js');
...
// 7. Инициализация после загрузки DOM
...
// 8. Service Worker для кэширования (опционально)
